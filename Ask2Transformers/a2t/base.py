...
class SentimentClassifier(Classifier):
    ...
    def _initialize(self, pretrained_model: str):
        ...
        self.model = AutoModelForSequenceClassification.from_pretrained(pretrained_model)
        self.config = AutoConfig.from_pretrained(pretrained_model)
        if pretrained_model == 'cardiffnlp/twitter-roberta-base-sentiment':
            self.ent_pos = self.config.label2id.get("LABEL_2", self.config.label2id.get("label_2", None))
        else:
            self.ent_pos = self.config.label2id.get("POS", self.config.label2id.get("pos", None))
        if self.ent_pos is None:
            raise ValueError("The model config must contain ENTAILMENT label in the label2id dict.")
        else:
            self.ent_pos = int(self.ent_pos)
...
__pdoc__ = {"SentimentClassifier.__call__": True}